...
import sh.okx.civmodern.common.mixins.ClickableStyleFinderAccessor;
...
public class LabelComponent extends BaseUIComponent {
    ...
    protected Style styleAt(int mouseX, int mouseY) {
        var clickHandler = new StyleCollector(this.textRenderer, this.x + mouseX, this.y + mouseY);
    ...
    /**
     * Vanilla's clickable-style search, redirected to record the style under the cursor
     * instead of dispatching a click. Inlined from owo's braid RawLabel.Instance.
     */
    protected static class StyleCollector extends ActiveTextCollector.ClickableStyleFinder {
        public StyleCollector(Font font, int clickX, int clickY) {
            super(font, clickX, clickY);
            ((ClickableStyleFinderAccessor) this).civmodern$setStyleScanner(
                ((ClickableStyleFinderAccessor) this)::civmodern$setResult);
        }
    }
